...
class MainUnitBasic:
    ...
    def __init__(self, dependencies: DependencyInjector):
        ...
        logger.debug('MainUnitBasic:initialized')
    
    
    # * Handle Tracked Videos
    def update_tracked_videos(self, video_feed_list: list[VideoFeed]):
        logger.debug(f'MainUnitBasic:removing {len(self._tracked_videos)} and adding {len(video_feed_list)}')
    ...
    def start_to_track_video(self, video_feed: VideoFeed):
        logger.debug(f'MainUnitBasic:adding {video_feed.id}')
        ...
        tracked_video.add_detector(video_detector,
            on_object_detection = lambda id, objects: logger.debug(f'MainUnitBasic:detection:{id}:{objects}'),
            on_error = lambda id, objects: logger.debug(f'MainUnitBasic:error:{id}:{objects}')
        )
        
        self._tracked_videos.append(tracked_video)
        logger.debug(f'MainUnitBasic:added {video_feed.id}')
        
        
    def remove_tracked_video(self, video_feed_id: str):
        logger.debug(f'MainUnitBasic:removing {video_feed_id}')
        for index, tracked_video in enumerate(self._tracked_videos):
            if tracked_video.video_feed.id == video_feed_id:
                ...
                break
        logger.debug(f'MainUnitBasic:removed {video_feed_id}')